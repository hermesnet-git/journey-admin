...
interface JourneySearchBoxProps {
  journeys: JourneySummary[] | null;
  loadError: string | null;
  query: string;
  onQueryChange: (value: string) => void;
...
// Extraído do corpo do toolbar pra ser reaproveitado também pelo card de busca da tela inicial
// (ExecutionsPage, estado ocioso) — mesmo autocomplete de jornada, só que hospedado num container
// diferente (o pai decide altura/borda; este componente só cuida do input + dropdown).
export function JourneySearchBox({ journeys, loadError, query, onQueryChange, selected, onSelect, placeholder }: JourneySearchBoxProps) {
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          placeholder={placeholder ?? 'Buscar jornada para executar...'}
          autoComplete="off"
      ...
      {open && (
        ...
        >
          {loadError ? (
            ...
            <div className="px-4 py-3">
              <Text size={13} color={skinVars.colors.textSecondary}>
                {q.length === 0 ? 'Nenhuma jornada publicada disponível.' : `Nenhuma jornada publicada encontrada para "${query}".`}
              </Text>
            </div>
          ) : (
            results.map((journey) => (
