...
export function FormPreviewNodeRenderer({ node }: { node: SduiNode }) {
  const children = (node.children ?? []).map((child) => <FormPreviewNodeRenderer key={child.id} node={child} />);
  const disabled = node.props.disabled === true || node.props.readOnly === true;

  switch (node.type) {
    case 'ui.screen':
      return <Stack space={24}>{text(node, 'title') && <Title2>{text(node, 'title')}</Title2>}<Stack space={24}>{children}</Stack></Stack>;
    case 'ui.container':
      return <div style={{ padding: spacing(node.props.paddingToken), background: suffix(node.props.backgroundToken).includes('primary') ? skinVars.colors.backgroundAlternative : undefined, borderRadius: spacing(node.props.borderRadiusToken) }}><Stack space={16}>{children}</Stack></div>;
    case 'ui.stack':
      return <div style={{ display: 'flex', flexDirection: node.props.direction === 'horizontal' ? 'row' : 'column', gap: spacing(node.props.spacingToken, 16), alignItems: String(node.props.alignment ?? 'stretch') as React.CSSProperties['alignItems'] }}>{children}</div>;
    case 'ui.card':
    ...
    case 'ui.text':
      return <Text size={suffix(node.props.variant) === 'caption' ? 12 : suffix(node.props.variant).includes('heading') ? 18 : 15} textAlign={node.props.align as 'left' | 'center' | 'right' | undefined}>{text(node, 'text', 'Texto')}</Text>;
    case 'ui.image':
      return text(node, 'source') ? <Image src={text(node, 'source')} alt={text(node, 'alt')} width="100%" /> : <div style={{ ...staticFieldStyle, textAlign: 'center' }}>{text(node, 'alt', 'Prévia da imagem')}</div>;
    case 'ui.icon': {
