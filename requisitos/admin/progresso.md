...
> ou business key, direto na própria tela, sem passar pelo Diagnóstico) uma instância `ACTIVE` já em
> andamento e voltar a interagir com ela ao vivo — útil quando o usuário fechou a aba, atualizou o
> navegador, ou saiu no meio de uma execução em espera. Reaproveita a resolução de versão/fluxo do
> Diagnóstico (`FlowVersionResolver`, extraído de `GetExecutionHistoryDetail`) e o resolvedor de
> passo atual já existente (`ExecutionStepResolver`) — nenhum mecanismo novo no motor, só um novo
> caminho de entrada pro que já existia. De quebra, corrigida uma regressão real encontrada na mesma
> sessão: REQ-05.08.005 (checagem de cadeia 100% síncrona antes de iniciar/completar/pular) tinha
> ficado só no caminho do canal digital (`ms-journey` → `ms-espec-registry`) depois da migração de
> Execução para admin/back em 2026-09-11 — portada de volta (`SynchronousChainCheck`, `domain/
> execution`). Também corrigidas ~21 células de evidência do FT-05 (e uma do FT-03/US-03.12) que
> ainda citavam classes removidas nessa mesma migração (`ms-espec-registry`/`SimulationController`/
> `CamundaClient`/`KafkaBridgeScheduler`), e reescritos REQ-14.02.003/REQ-14.04.003 (FT-14): o teste
> de conexão do catálogo de integrações roda no admin-back e resolve a credencial dinamicamente via
> Key Vault (nunca por configuração estática), lendo o segredo em memória sem nunca persisti-lo/
> exibi-lo/logá-lo — o texto antigo dizia o oposto ("admin-back nunca acessa o Key Vault"), o que já
> não era mais verdade desde a migração de ontem. Total do FT-05: 58 → 62 REQs.
...
>
> **Mudança arquitetural (FT-03/FT-04, 2026-08-24): `formId` associado à User Task removido, substituído por `embeddedScreen` desenhado direto no nó.** Causa raiz: a Runtime Engine só suporta um conjunto básico de tipos de campo nativos (~5-6), inviabilizando um formulário do catálogo ser interpretado por ela — o Admin Portal sempre precisou resolver a tela sozinho (SDUI), então manter uma User Task *associada* a um `Form` por `formId` não trazia benefício real e ainda travava o catálogo num punhado de tipos simples. `FlowNode` ganhou `embeddedScreen` (tela editável, desenhada no editor de fluxo) e `embeddedScreenSdui` (árvore compilada, só em snapshot de publicação/versão — nunca ambos preenchidos ao mesmo tempo); `Form`/`FormField` (catálogo) viraram só um modelo de cópia opcional ("Importar formulário"/"Salvar como formulário reutilizável" em `FormPreviewDock.tsx`), nunca mais uma referência persistida. Catálogo de tipos de campo ampliado de 5 para 17 (REQ-04.02.011 a 022: `SECTION`, `RADIO`, `SWITCH`, `SLIDER`, `RATING`, `STEPPER`, `AUTOCOMPLETE`, `TITLE`, `IMAGE`, `DIVIDER`, `CARD`, `CALLOUT`), com editor drag-and-drop completo (`FormScreenCanvas.tsx`, dnd-kit) — US-03.16 renomeada e upgradada de `in_progress` pra `done`. Nome técnico do campo (REQ-04.01.007) passou a ser editável na tela embutida (não mais só no catálogo), com unicidade verificada na jornada inteira (REQ-03.09.011 explicitado). Bugs reais corrigidos no caminho: `{{variavel}}` não era resolvido dentro de uma tela real desenhada (só funcionava sem tela, REQ-05.02.005); `defaultValue` com `{{variavel}}` passou a pré-preencher o campo de verdade (REQ-04.04.002); nome de campo duplicado entre User Tasks diferentes não era barrado na hora de criar, só no salvar. Publicação: `forms`/lista de formulários removida do snapshot enviado ao runtime e da inspeção (REQ-02.09.002/REQ-02.10.001/REQ-06.02.004); novo REQ-02.09.005 — número da versão publicada gravado como tag de versão do processo implantado, distinta do contador de implantação interno do runtime (a tela de inspeção do runtime mostrava só esse contador, sem jeito de saber qual versão do Admin Portal estava ativa), exibido também no Executor (REQ-05.07.004 novo). Outras evoluções da mesma sessão, sem relação com a mudança acima: geração de fluxo por IA passou a considerar o fluxo já desenhado como contexto, preservando id/posição/tela de nós não afetados por um pedido aditivo (REQ-03.17.006 novo — antes todo pedido, mesmo "adicione uma tarefa", apagava a jornada inteira); id da jornada exibido (somente leitura) no painel de Propriedades quando nenhum nó está selecionado (REQ-02.05.005 novo); conexão do editor de fluxo agora pode ser reconectada arrastando a ponta pra outro nó, sem excluir e redesenhar (REQ-03.02.003, comportamento já coberto pelo texto genérico existente). Total do FT-02: 42 → 44 REQs; FT-03: 85 → 96 REQs (2 `in_progress` viraram `done`); FT-04: 25 → 37 REQs; FT-05: 43 → 44 REQs.
...
| [x] | REQ-02.03.006 | O sistema deve permitir agrupar a listagem de jornadas por produto, por produto e canal, por canal, ou sem agrupamento algum. | done | front: `JourneysPage.tsx` — `GroupMode`/`GROUP_MODE_OPTIONS`, dropdown "Agrupar" com os 4 modos (`groupKeyFor`), cabeçalho de grupo com contagem | achado nesta sessão: já estava implementado por completo desde o commit `689466d` (2026-08-16), a nota de `in_progress` nunca foi atualizada |
| [x] | REQ-02.03.007 | O sistema deve permitir ordenar a listagem de jornadas, em ordem crescente ou decrescente, pelos campos jornada (nome), canal, status ou data de atualização. | done | front: `JourneysPage.tsx` — `SortField`/`SortHeader`/`toggleSort`, colunas clicáveis (Jornada/Canal/Status/Atualizada em) com ícone de direção | achado nesta sessão: ordenação client-side já implementada por completo desde o commit `689466d` (2026-08-16); a nota de `todo` nunca foi atualizada |
...
| [x] | REQ-03.09.011 | O nome de cada variável de saída — de integração ou de campo de tela embutida de User Task — deve ser único no escopo da jornada e seguir a mesma regra de nome técnico dos campos de formulário (REQ-04.01.007). | done | back: `FlowValidator` — `seenOutputNames` acumula `startVariables`, `outputMapping` e nomes de `embeddedScreen` de toda User Task, rejeitando (422) repetição; front: `FormPreviewDock.tsx`/`formScreenModel.ts` (`otherTaskFieldNames`) evita a colisão já na hora de criar/renomear um campo no editor de tela | front passou a validar formato/unicidade cross-node nesta sessão (2026-08-24), além da checagem de duplicidade que já existia |
...
| [x] | REQ-03.11.009 | A condição do gateway pode referenciar a variável reservada `channel`, injetada automaticamente pelo tipo de canal que inicia a instância, nunca declarável pelo usuário no nó START. | done | back: `FlowValidator.CHANNEL_VARIABLE` — sempre presente em `startVariableNames`, rejeita (422) qualquer `startVariables`/`outputMapping` que declare o nome `channel`; `StartExecution` (admin/back) e `JourneyController.start()` (ms-journey) injetam a variável real a partir do `?channel=` recebido | requisito novo desta sessão — cobre a jornada multicanal (US-02.02/FT-05) |
...
| [x] | REQ-03.12.004 | O endpoint de início de instância deve aceitar um mapa de valores no corpo e recusar a chamada se faltar alguma variável declarada. | done | admin/back: `ExecutionController.start` aceita `@RequestBody Map<String,Object> variables`; `AnswerConversion.fromDeclaredVariables` (chamado por `StartExecution`) valida presença + coerciona pelo tipo declarado | |
| [x] | REQ-03.12.005 | Valores extras informados que não correspondam a nenhuma variável declarada são aceitos e repassados sem erro. | done | admin/back: `AnswerConversion.fromDeclaredVariables` inclui as chaves não-declaradas, tipadas por inferência simples do tipo Java recebido do Jackson (Boolean/Number/String) | |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-03.16.001 | Ao selecionar, no canvas, uma `USER_TASK`, o editor deve exibir automaticamente um dock com o editor da tela embutida do nó (`embeddedScreen`), ancorado à base do canvas, sem exigir uma ação dedicada de clique. | done | front: `FormPreviewDock.tsx` — `JourneyDesignerPage.tsx` deriva `previewNode` de qualquer `USER_TASK` selecionada (não depende mais de `data.formId`); paleta (`FormFieldPalette.tsx`), canvas arrastável (`FormScreenCanvas.tsx`, dnd-kit) e painel de configuração (`FormFieldConfigPanel.tsx`) com componentes reais da Mística — mesma renderização da tela de Execução (`SduiFormRenderer.tsx`) | reescrito nesta sessão (2026-08-24) — antes dependia de `formId`/preview estático; upgradado de `in_progress` pra `done`, editor de verdade implementado |
| [x] | REQ-03.16.002 | Ao selecionar qualquer outro elemento do canvas, o dock deve deixar de ser exibido. | done | front: `previewNode` volta a `null` quando `propertiesNode` não é uma `USER_TASK` | upgradado de `in_progress` pra `done` nesta sessão (2026-08-24) |
...
| [x] | REQ-04.01.005 | Toda User Task deve nascer com uma tela `ui.screen`, inclusive quando sua finalidade for apenas informativa. | done | front: `JourneyDesignerPage.tsx` cria a raiz pelo catálogo nos caminhos da paleta e da criação rápida; back: `FlowValidator.validateEmbeddedScreen` rejeita tarefa sem tela; `ms-espec-registry`: `StepResolver` e `FormSpecController` rejeitam snapshot publicado sem tela | revisado em 2026-09-09 — removida a síntese tardia de tela a partir de mensagem avulsa; não há compatibilidade retroativa para resíduos de teste sem tela |
...
> User story inteira removida em 2026-09-05 junto com o catálogo de Formulários — ver nota de remoção no topo deste documento.

| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [~] | ~~REQ-04.03.001~~ | ~~O sistema deve permitir usar um formulário do catálogo como ponto de partida (cópia) para telas de User Tasks em múltiplas jornadas.~~ | removido | | não conta mais no total de requisitos |
| [~] | ~~REQ-04.03.002~~ | ~~O sistema deve permitir usar um formulário do catálogo como ponto de partida (cópia) para telas de múltiplas User Tasks.~~ | removido | | não conta mais no total de requisitos |
...
A execução roda contra o motor de runtime real: `admin/back` (pacotes `application`/`domain`/`infrastructure`/`interfaces/execution`) fala direto com a REST API do motor via `RuntimeEngineMonitoringAdapter`, chamado pela aba "Execuções" do `admin/front` (`front/src/execution/`); o `ms-espec-registry` (`simulacoes/ms-espec-registry`) ficou só com o catálogo SDUI, sem nenhuma responsabilidade de execução. As Service/Receive Tasks do fluxo rodam como Java Delegates dentro do próprio motor, no serviço separado `ms-runtime-camunda` (`simulacoes/ms-runtime-camunda`): conectores REST via `HttpConnectorDelegate` (apontando pra `ms-mock-api-rest`, que emula as respostas reais) e conectores Kafka via `KafkaConnectorWorker` — um único `@Scheduled` que publica e consome automaticamente contra um broker Kafka real local, sem mock.

### US-05.01 Execução

| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-05.01.001 | O sistema deve permitir executar jornadas. | done | `StartPanel.tsx` (botão "Executar") → `POST /api/v1/journeys/{id}/instances` (admin/back, `StartExecution`) → `RuntimeEngineMonitoringAdapter.startProcessInstance` → `POST /process-definition/key/{key}/start` no motor de runtime | |
...
| [x] | REQ-05.02.005 | O sistema deve apresentar a tela obrigatória da User Task com referências e valores vinculados resolvidos a partir das variáveis atuais do processo. | done | `ms-espec-registry`: contexto de runtime do envelope publicado; front: renderer usa o valor resolvido como valor inicial editável | revisado em 2026-09-09 para remover o caso obsoleto de User Task sem tela |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-05.04.001 | O sistema deve executar a jornada publicada contra o motor de runtime real, não um motor simplificado interno ao Admin Portal. | done | `admin/back`: `RuntimeEngineMonitoringAdapter` chama a REST API real do Camunda 7 (`engine-rest`), a mesma que `ms-transform-publication` usa para implantar | Exige jornada publicada — ver ajuste no Objetivo da feature em `ej-admin-requisitos.md` |
...
| [x] | REQ-05.04.003 | As integrações Kafka referenciadas pelas jornadas devem executar contra um broker Kafka real, com publicação e consumo de mensagens efetivos. | done | `KafkaConnectorWorker.tick()` (`ms-runtime-camunda`) — `@Scheduled` único que produz (fetchAndLock em lote + `KafkaTemplate.send`, `publishAndComplete`) e consome (`KafkaConsumer` inscrito nos tópicos descobertos por `KafkaConsumerNodeDiscovery`), contra um broker Kafka 4.3.1 local (KRaft) | Verificado ao vivo publicando/consumindo mensagem real em ambos os sentidos |
| [x] | REQ-05.04.004 | Toda instância deve ser iniciada informando um tipo de canal, validado contra os tipos habilitados na jornada publicada; o motor deve receber esse valor como variável de processo reservada `channel`. | done | back: `StartExecution.execute()` (admin/back) e `JourneyController.start()` (ms-journey) exigem `?channel=`, rejeitam com `UnsupportedChannelException` se fora de `channelTypes()`, e injetam `startVariables.put("channel", ...)` antes de iniciar a instância no motor | requisito novo desta sessão |

### US-05.05 Etapas de integração

| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-05.05.001 | O sistema deve permitir avançar manualmente uma etapa de integração (Service Task ou Receive Task) que dependeria de um evento assíncrono externo, pulando sua conclusão. | done | Botão "Pular etapa" (`DevicePreview.tsx`) → `POST /instances/{id}/simulate-step` (nome do endpoint em `ExecutionController`, admin/back, fora de escopo — o front chama isso via `skipStep()`) → `SkipStep.execute()` completa o external task (Kafka) ou correlaciona mensagem (`RECEIVE_TASK`) direto no motor | |
...
| [x] | REQ-05.06.003 | O sistema deve apresentar o resultado das integrações já executadas (dados retornados/mapeados por Service/Receive Tasks). | done | Aba Log (`InspectorPanel.tsx`) — cada entrada do trail (`TrailEntry.url`/`response`, computado em `ExecutionStepResolver.buildTrail`, admin/back) é anexada como bloco JSON abaixo da mensagem daquele nó (`trailLogData`) | evidência revisada: a aba "Integrações" dedicada que existia até então foi removida por redundância (mesmo dado, consolidado na aba Log) |
| [x] | REQ-05.06.004 | O sistema deve apresentar um log cronológico dos passos executados durante a execução. | done | Aba Log — acumulado 100% no front a cada `start`/`complete-task`/`skip-step`, sem endpoint dedicado, somado ao `trail` computado no backend (`StartExecution`/`CompleteExecutionTask`/`SkipStep`, admin/back, via `ExecutionStepResolver.resolve(..., since)`) para os nós resolvidos numa única chamada síncrona sem wait state — o front usa `initialStep.trail` para preencher o log inicial nesse caso | corrigido bug em que `start()` não populava `trail` e o front ignorava `initialStep.trail`, deixando o log vazio quando a jornada rodava inteira de uma vez |
...
| [x] | REQ-05.07.007 | Quando uma jornada tiver mais de uma versão publicada simultaneamente, o sistema deve permitir escolher qual versão executar antes de iniciar; com uma única versão publicada, deve usá-la automaticamente. | done | back: `StartExecution.resolveTarget` usa `JourneyVersionRepository` quando uma versão é informada (valida `PUBLISHED`); `RuntimeEngineMonitoringAdapter.startProcessInstance` resolve o `processDefinitionId` via `versionTag` antes de iniciar. front: `StartPanel.tsx` — seletor de versão exibido só quando `publishedVersions.length > 1` (`listJourneyVersions`) | Testado em runtime: instâncias iniciadas com/sem `version` confirmadas rodando nos deployments corretos (v1 vs v3) direto no motor |

### US-05.08 Tratamento de falhas de integração

| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-05.08.001 | O sistema deve detectar quando uma etapa de integração (Service Task ou Receive Task) falha durante a execução (ex.: conector REST inacessível) e identificar qual nó do fluxo causou a falha, mesmo quando o motor não expõe isso diretamente (a transação dá rollback antes de qualquer histórico ser gravado). | done | `FlowGraph.nextConnectorNodeAfter()` + `ExecutionErrorAttribution.attribute()` (admin/back) — captura `RestClientException` de `CompleteExecutionTask`/`SkipStep` e segue as conexões do fluxo a partir do passo atual até o próximo nó com conector (único tipo capaz de falhar assim), devolvendo `errorNodeId`/`errorNodeName`/`errorMessage` numa resposta 200 estruturada em vez do 500 cru do motor | Validado ao vivo derrubando `ms-mock-api-rest` e completando uma task real: a resposta veio com o nó de serviço correto, não a User Task anterior |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-05.09.001 | Cada execução deve possuir um identificador de correlação (business key) próprio, gerado automaticamente ao iniciar a instância. | done | `StartExecution.execute()` (admin/back) gera `UUID.randomUUID()` e passa como `businessKey` em `runtimeExecutionPort.startProcessInstance(...)` | |
| [x] | REQ-05.09.002 | Uma Service Task com conector Kafka deve publicar a mensagem de verdade num broker Kafka real, automaticamente, sem exigir ação manual. | done | `KafkaConnectorWorker.produceTick()` (`ms-runtime-camunda`) — trava em lote os external tasks Kafka pendentes (fetchAndLock), resolve `{{variável}}` e publica via `KafkaTemplate` (`publishAndComplete`) | Verificado ao vivo: jornada IoT, mensagem real chegou no tópico em ~3s sem clique |
...
| [x] | REQ-05.09.004 | O sistema deve detectar e processar automaticamente uma mensagem Kafka publicada no tópico de uma Receive Task ou de um início por mensagem em execução, avançando a instância sem exigir ação do usuário — inclusive quando publicada por um produtor externo ao Admin Portal. | done | `KafkaConnectorWorker.consumeTick()` (`ms-runtime-camunda`) — `KafkaConsumer` inscrito nos tópicos descobertos (`KafkaConsumerNodeDiscovery.discover()`), correlaciona por businessKey (Receive Task) ou inicia direto pela definição de processo (Message Start Event) | Verificado ao vivo com `kafka-console-producer` (produtor externo, sem passar pelo painel do Admin Portal) |
| [x] | REQ-05.09.005 | O sistema deve permitir publicar uma mensagem de teste real, com tópico (somente leitura) e payload editável em JSON, na tela de execução, para uma Receive Task que dependa de mensagem Kafka. | done | `SendTestMessagePanel.tsx` → `POST /journeys/{id}/nodes/{nodeId}/test-message` (admin/back, `SendTestMessage.execute()`) → `KafkaTemplate.send` direto no tópico do nó | |
...
| [x] | REQ-05.09.007 | Uma jornada cujo início é por mensagem deve oferecer, na tela de busca, o painel de envio de mensagem de teste para iniciar uma instância nova, sem pré-preencher a business key. | done | `StartPanel.tsx` — busca o nó de início via `GET /journeys/{id}/execution-flow`; se `MESSAGE_START_EVENT`, troca o botão "Executar" pelo `SendTestMessagePanel` com `initialPayload={{}}` | |
| [x] | REQ-05.09.008 | Depois de enviar a mensagem de teste que inicia uma jornada por mensagem, o sistema deve aguardar automaticamente até a instância nova aparecer e prosseguir para a tela de execução. | done | `StartPanel.tsx` (`pollForNewInstance`) → `GET /journeys/{id}/latest-instance?since=...` (admin/back, `GetLatestInstance` → `RuntimeEngineMonitoringAdapter.findMostRecentInstanceStartedAfter`, via histórico do motor de runtime) | |
...
| [x] | REQ-05.09.010 | Ao iniciar uma execução, o sistema deve permitir optar por controle manual das mensagens Kafka daquela instância, retirando suas Service Tasks Kafka do disparo automático do worker em background. | done | front: `StartPanel.tsx` — checkbox `manualKafkaControl`, passado a `startInstance(journeyId, variables, manualKafkaControl)`; back: `StartExecution.execute()` (admin/back) seta a variável de processo `KafkaVariableNames.MANUAL_CONTROL`, que `KafkaConnectorWorker.produceTick()` (`ms-runtime-camunda`) respeita para não publicar automaticamente | |
| [x] | REQ-05.09.011 | Com controle manual ativo, a tela de execução deve permitir publicar a mensagem de uma Service Task Kafka digitando o payload manualmente ou gerando-o automaticamente a partir do mapeamento configurado. | done | front: `KafkaManualSendPanel.tsx` (modo "escolher" → digitar manualmente ou gerar); back: `SendKafkaMessage`/`PreviewKafkaMessage` (admin/back), endpoint dedicado de envio manual em `ExecutionController` | |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-05.10.001 | Ao selecionar uma Tarefa de Serviço ou de Recebimento, o painel deve apresentar a configuração do conector (tipo; REST: método/URL/headers/body; tópico: nome + cluster + Producer/Consumer). | done | `InspectorPanel.tsx` — `ConnectorConfigSection`/`RestConnectorDetails`/`TopicConnectorDetails`, alimentadas por `FlowNodeInfo.connectorConfig` | |
| [x] | REQ-05.10.002 | Ao selecionar um Gateway, o painel deve apresentar as condições de cada saída, destacando a percorrida. | done | `InspectorPanel.tsx` — `GatewaySection`, mesmo critério de "aresta percorrida" já usado pra colorir o diagrama (`visitedNodeIds`/`currentNodeId`) | Sem instância associada (fluxo estático), nada aparece marcado como tomado |
| [x] | REQ-05.10.003 | Ao selecionar o nó de Início, o painel deve apresentar as variáveis de entrada declaradas com o valor real informado na execução. | done | `InspectorPanel.tsx` — `StartVariablesSection`, valor lido de `variables` (mesma lista da aba Variáveis) por nome | Mostra "—" quando o valor ainda não foi resolvido |
...
| [x] | REQ-02.06.004 | Uma jornada pode possuir mais de uma versão publicada simultaneamente, cada uma associada a uma versão imutável e a um deployment próprio no runtime — publicar uma versão nova não invalida nem despublica a anterior, para não interromper instâncias já em execução nela. Alterações realizadas após a publicação não devem modificar o snapshot publicado; para disponibilizá-las, o usuário deve publicar uma nova versão. | done | back: `journey_version.runtime_deployment_id` (um por versão) + `PublishJourneyVersion.goLive` não despublica a versão anterior; `journey_publication` continua único por jornada (snapshot mais recente pra inspeção), sem relação com quantas versões estão `PUBLISHED`; ver FT-06 | requisito revisado (2026-09-07): antes exigia no máximo uma publicação ativa por jornada — corrigido pra permitir que instâncias da versão antiga continuem rodando sem interrupção ao publicar uma versão nova |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-02.10.001 | Para uma jornada com publicação ativa (`PUBLISHED`), o sistema deve permitir visualizar o JSON completo enviado à API de publicação do runtime (produto, canal, fluxo — com a árvore SDUI já compilada da tela de cada User Task), por meio de uma ação na listagem de jornadas ao lado de "Editar" e "Excluir". | done | back: `GET /api/v1/journeys/{id}/publication` (`GetPublicationSnapshot` + `PublicationSnapshotRecord.from`, 409 se a jornada não estiver `PUBLISHED`); front: ícone "Ver publicação" (`FileJson`) em `JourneyActions`, abre `PublicationSnapshotModal` com o JSON completo e botão "Copiar JSON" | testado via curl: 200 com JSON completo em jornada `PUBLISHED`, 409 em jornada não publicada; escopo mais restrito que o `REQ-06.03.006` removido (só a publicação ativa da jornada, não qualquer versão histórica); evidência atualizada nesta sessão (2026-08-24) — `forms` removido do snapshot |
...
| [x] | REQ-06.02.004 | A nova versão deve possuir cópia independente do fluxo, conexões e das telas embutidas (`embeddedScreen`/`embeddedScreenSdui`) de cada User Task. | done | back: `JourneySnapshotFactory` gera snapshot JSONB independente por versão; `JourneyVersionRepositoryAdapter` compila `embeddedScreenSdui` por nó via `PublicationSnapshotRecord.embeddedScreenSduiOf` | evidência atualizada nesta sessão (2026-08-24) — `resolveForms` removido, sempre retorna lista vazia |
...
| [x] | REQ-06.04.004 | Ao publicar uma nova versão, qualquer versão anteriormente publicada da mesma jornada permanece `PUBLISHED` e com seu deployment intacto no runtime — publicar não é um efeito colateral de despublicar. Uma jornada pode ter mais de uma versão `PUBLISHED` ao mesmo tempo. | done | back: `PublishJourneyVersion.goLive` não despublica mais a versão `PUBLISHED` anterior (bloco removido) | requisito revisado (2026-09-07): antes marcava a anterior como `UNPUBLISHED` ao publicar — corrigido porque isso mentia sobre o estado real do runtime (o Camunda já mantinha a versão antiga deployada e rodando; só o rótulo no admin mudava, podendo até derrubar instância ativa se alguém depois despublicasse "a" versão errada) |
| [x] | REQ-06.04.005 | O sistema deve preservar o snapshot de toda versão publicada, atual ou anterior. | done | back: cada `journey_version` mantém sua linha/`version_snapshot` intactos independente do status | |
| [x] | REQ-06.04.006 | A publicação deve registrar qual versão foi enviada ao runtime, incluindo o identificador do deployment gerado (necessário para despublicar essa versão especificamente, sem afetar outras — ver REQ-06.04.010). | done | back: `journey_version.runtime_deployment_id`, capturado do retorno de `RuntimePublicationPort.publish` (`ms-transform-publication` já devolvia `deploymentId` em `PublicationController.PublishResponse`, só não era lido) | |
| [x] | REQ-06.04.007 | A jornada deve indicar, como referência rápida, a mais recente entre suas versões atualmente publicadas; a listagem de versões deve exibir o status real de cada uma individualmente. | done | back: `JourneyViewAssembler` escolhe a `PUBLISHED` de maior `versionNumber` (`findAllByJourneyIdAndStatus` + `max`); front: grid de versões mostra o status de cada linha | |
...
| [x] | REQ-06.04.009 | Ao despublicar uma jornada, toda versão `PUBLISHED` dela deve ser marcada como `UNPUBLISHED`, uma a uma, cada uma despublicando apenas o próprio deployment no runtime (nunca o de outra versão), preservando os snapshots; a jornada deixa de indicar uma versão atualmente publicada. Se qualquer uma dessas versões tiver instância de processo ativa (REQ-06.04.014), a operação para nessa versão e as demais já processadas antes dela permanecem despublicadas. | done | back: `UnpublishJourney.execute` itera `findAllByJourneyIdAndStatus(PUBLISHED)`, chamando `RuntimePublicationPort.unpublish(journeyId, version.getRuntimeDeploymentId())` por versão | requisito revisado (2026-09-07): antes despublicava só "a" versão publicada, assumindo no máximo uma; agora itera todas, e cada despublicação mira só o próprio deployment |
| [x] | REQ-06.04.010 | O sistema deve permitir despublicar a versão atualmente `PUBLISHED` de uma jornada diretamente pela versão, afetando apenas o deployment dela no runtime; a despublicação de uma versão só deve refletir no status da jornada (`UNPUBLISHED`) quando não restar nenhuma outra versão publicada — se outra versão da mesma jornada continuar `PUBLISHED`, a jornada permanece `PUBLISHED`. | done | back: `UnpublishJourneyVersion` agora autocontido (não delega mais em `UnpublishJourney`) — despublica só o próprio deployment via `version.getRuntimeDeploymentId()`, e só flipa `Journey.status` se `findAllByJourneyIdAndStatus(PUBLISHED)` ficar vazio; front: botão "Despublicar" na linha da versão em `JourneysPage` | requisito revisado (2026-09-07): antes sempre derrubava o status da jornada inteira; corrigido pra jornada continuar `PUBLISHED` enquanto sobrar outra versão publicada |
| [x] | REQ-06.04.011 | O sistema deve permitir republicar qualquer versão `UNPUBLISHED` de uma jornada (não apenas a mais recente), sem alterar seu conteúdo/snapshot, retornando-a a `PUBLISHED` e refletindo no status da jornada, que volta a `PUBLISHED`. Uma versão `PUBLISHED` já existente na jornada, se houver, permanece intacta (mesmo comportamento de REQ-06.04.004). `INACTIVE` (jornada excluída) continua fora de alcance (REQ-06.05.004). | done | back: `POST /journeys/{id}/versions/{versionId}/republish` + `RepublishJourneyVersion` reaproveita `PublishJourneyVersion.goLive(...)` — mesma lógica de validar canal/produto ativos e publicar no runtime, sem despublicar nenhuma outra versão | requisito revisado (2026-09-07): removida a cláusula que despublicava a `PUBLISHED` atual antes de republicar (mesma correção de REQ-06.04.004) |
| [x] | REQ-06.04.012 | Antes de republicar uma versão, o sistema deve informar ao usuário que ela volta a ficar publicada e que outras versões publicadas da jornada não são afetadas, solicitando confirmação antes de prosseguir. | done | front: `ConfirmDialog` em `JourneyVersionsRows` com mensagem única (removida a branch condicional `hasPublishedVersion`, que não fazia mais sentido) | requisito revisado (2026-09-07): mensagem antiga avisava que a versão publicada atual seria substituída — não é mais verdade |
...
| [x] | REQ-06.04.014 | O sistema não deve permitir despublicar uma versão que possua uma ou mais instâncias de processo ativas (em execução) no runtime — a operação deve ser bloqueada antes de qualquer remoção de deployment, com uma mensagem clara informando a quantidade de instâncias ativas, para nunca interromper silenciosamente quem já está no meio de uma jornada. | done | `ms-transform-publication`: `CamundaRestClient.deleteDeployment` checa `activeInstanceCountForDeployment` antes de deletar (`cascade=true`), lança `ActiveInstancesExistException` (409 `ACTIVE_INSTANCES_EXIST`) se houver alguma; `admin/back`: `PublicationAdapter.unpublish` propaga como `RuntimeUnpublishBlockedException`; front: mensagem distinta por `code` em `JourneysPage` | corrige bug real relatado pelo usuário: era possível despublicar uma jornada com instância ativa, matando-a silenciosamente (o delete do deployment usava `cascade=true`, que apaga runtime e histórico junto) |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [~] | REQ-14.04.001 | O sistema deve permitir disparar um teste de conexão para um par cluster + credencial, validando alcançabilidade e credencial. | in_progress | back: `POST /api/v1/credential-references/{id}/connection-test` → `TestCredentialConnection` → `KafkaConnectionTestAdapter` (`AdminClient.describeCluster()`, dentro do próprio admin-back desde a migração de 2026-09-11); front: botão "Testar conexão" em `CatalogPage`/`ConnectorWizard` | mecanismo completo, mas só valida de verdade contra `KAFKA` (testado contra o broker local, sem autenticação); `EVENT_HUBS`/`SERVICE_BUS` retornam "não suportado neste ambiente" de forma explícita — falta dependência do SDK da Azure (nenhuma no projeto hoje) e um ambiente Azure real pra fechar essa ponta |
| [x] | REQ-14.04.002 | O teste de conexão deve se limitar a metadado, nunca publicar ou consumir uma mensagem real. | done | back: `KafkaConnectionTestAdapter` usa só `AdminClient.describeCluster()` — nunca instancia `KafkaProducer`/`KafkaConsumer` | |
| [x] | REQ-14.04.003 | A execução do teste deve ocorrer no admin-back; a credencial deve ser resolvida dinamicamente via Key Vault, nunca por configuração estática por ambiente. | done | back: `KafkaConnectionTestAdapter`/`KafkaTopicListingAdapter` (admin-back) chamam `CredentialResolver` — hoje `LocalCredentialResolver` (stub vazio, broker local sem autenticação); substituível por uma implementação real do Key Vault sem mudar quem chama | migrado de `ms-espec-registry` em 2026-09-11; resolução real do Key Vault (ambiente corporativo) ainda pendente — falta dependência Azure e ambiente pra validar |
| [x] | REQ-14.04.004 | O resultado deve indicar sucesso ou falha traduzida para uma causa reconhecível, nunca o erro cru. | done | back: `KafkaConnectionTestAdapter`/`ConnectionTestResult` traduzem `TimeoutException`/`ExecutionException` em mensagem amigável; nunca propagam stacktrace | |
...
Tela separada de Execução (`front/src/diagnostics/DiagnosticoPage.tsx`), item de menu próprio, pra investigar o comportamento de qualquer execução de jornada no motor de runtime — iniciada pelo Admin Portal ("Executar") ou por um canal digital — sem distinção de origem entre elas (mesmo motor de runtime, REQ-05.04.001). Reaproveita `HistoryWorkspace.tsx`/`InspectorPanel.tsx` (o mesmo painel de observabilidade da Execução, US-05.10) pro detalhe de uma execução, e `searchInstanceHistory`/`getInstanceHistory` (`execution/api.ts`, `InstanceHistoryController.java` em `ms-espec-registry`) pra busca/detalhe — endpoints que já existiam antes desta feature.

### US-15.01 Busca de execuções

| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-15.01.001 | O sistema deve permitir buscar execuções por jornada, por business key ou por instance ID, com o usuário escolhendo explicitamente o tipo de busca. | done | `DiagnosticoPage.tsx` — `SearchTypeToggle` (Jornada/Business key/Instance ID), cada tipo troca o campo e a chamada de busca correspondente | |
| [x] | REQ-15.01.002 | A busca por jornada deve oferecer um autocomplete das jornadas publicadas e filtrar as execuções pela jornada selecionada. | done | `DiagnosticoPage.tsx` — `JourneySearchBox` (`ExecutionToolbar.tsx`, extraído pra reuso), `handleSelectJourney` dispara `searchInstanceHistory({ journeyId })` | |
| [x] | REQ-15.01.003 | A busca por business key deve filtrar exatamente pelo valor informado. | done | `DiagnosticoPage.tsx` — `handleSearchClick` (tipo `businessKey`) chama `searchInstanceHistory({ businessKey })`, filtro server-side (`InstanceHistoryController.search`) | |
| [x] | REQ-15.01.004 | A busca por instance ID deve levar diretamente ao detalhe da execução, sem passar pela listagem. | done | `DiagnosticoPage.tsx` — `handleInstanceIdSearch` chama `getInstanceHistory(id)` direto, sem listar | |
| [x] | REQ-15.01.005 | Ao buscar por um instance ID que não exista, o sistema deve apresentar a mensagem de erro na própria tela de busca, sem navegar para a tela de detalhe. | done | `DiagnosticoPage.tsx` — `handleInstanceIdSearch` só chama `setSelectedId` no `.then` (sucesso); no `.catch`, seta `loadError` e permanece na tela de busca | corrigido nesta sessão — antes navegava pro detalhe e mostrava o erro lá |
| [x] | REQ-15.01.006 | O sistema deve permitir filtrar a busca por período (data de início). | done | `DiagnosticoPage.tsx` — campos `from`/`to` (`<input type="date">`); `searchInstanceHistory` (`execution/api.ts`) converte pra início/fim do dia em ISO antes de montar a query (`startOfDayInstant`/`endOfDayInstant`) | corrigido nesta sessão — o backend espera `java.time.Instant` completo, não `AAAA-MM-DD` |
| [x] | REQ-15.01.007 | A tela de Diagnóstico não deve apresentar nenhuma listagem de execuções antes de uma busca ser realizada. | done | `DiagnosticoPage.tsx` — `hasSearched` começa `false`, sem fetch automático no mount; `BlankState` (caixa tracejada) até a primeira busca | |

### US-15.02 Listagem de execuções

| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-15.02.001 | A listagem deve agrupar as execuções por jornada e versão por padrão, com opção de desagrupar e ver a lista plana. | done | `DiagnosticoPage.tsx` — `groupByVersion` (chave jornada+versão), toggle "Agrupado por versão"/"Agrupar por versão"; `version` novo em `HistoricInstanceSummary` (`processDefinitionVersion` do próprio Camunda, sem chamada extra) | back: `HistoricInstanceSummary.java`/`InstanceHistoryController.java` (`ms-espec-registry`) |
| [x] | REQ-15.02.002 | A listagem deve permitir ordenar pela data/hora de início, de forma crescente ou decrescente. | done | `DiagnosticoPage.tsx` — cabeçalho "Início" clicável, `sortDir`, ordenação client-side sobre o resultado já retornado | |
| [x] | REQ-15.02.003 | Cada execução listada deve indicar seu estado (em execução, concluída ou encerrada) com destaque visual. | done | `DiagnosticoPage.tsx` — `InstanceRow`/`bucketOf`/`bucketMeta` (pill colorida com ícone); `HistoryWorkspace.STATE_LABEL` (`ACTIVE` → "Em execução") | |
| [x] | REQ-15.02.004 | A listagem deve cobrir execuções originadas tanto pela funcionalidade Executar do Admin Portal quanto por canais digitais, sem distinção de origem entre elas. | done | Satisfeito por arquitetura, não por filtro dedicado: `searchInstanceHistory`/`InstanceHistoryController.search` consulta o histórico do motor de runtime direto — mesmo motor pra qualquer origem que iniciou a instância (`SimulationController`/admin ou `ms-journey`/canal digital), sem marcação de origem gravada nem necessária | businessKey é UUID puro nos dois caminhos, indistinguível de propósito — não há necessidade de diferenciar |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-15.04.001 | O Diagnóstico deve ser uma funcionalidade separada da Execução, acessível por item de menu próprio. | done | `Sidebar.tsx` — item "Diagnóstico" (`Stethoscope`) separado de "Execução"; `shell/types.ts` — `TabKind` ganhou `'diagnostico'`; `App.tsx` — aba/rota própria (`DIAGNOSTICO_TAB`) | |
| [x] | REQ-15.04.002 | A tela de Execução não deve oferecer busca ou consulta de execuções passadas — cobre apenas a execução ao vivo em andamento. | done | `ExecutionsPage.tsx`/`ExecutionToolbar.tsx` — modo Histórico removido por completo (`ModeToggle`, `HistorySearchBar`, `HistoryWorkspace` desligados desta tela); título da aba "Execução & Diagnóstico" → "Execução" | |
...
| 2026-08-23 (não commitado) | Auditoria da documentação de modelo de dados/API do admin contra o código real (back/front/`ms-espec-registry`), sem mudança de código nem de status de REQ. Removida a entidade órfã Execution Run/Step/Result — nunca teve tabela própria; a execução real roda inteiramente contra o motor de runtime, sem persistência própria no admin/back, e o único vestígio é um Audit Event genérico `EXECUTION_START` — presente em `ej-admin-modelo-dados-conceitual.md`, `ej-admin-modelo-dados-fisico.md`, `ej-admin-dicionario-dados.md`, `ej-admin-index.md`, `ej-admin-arquitetura-logica.md` e `ej-admin-openapi.yaml`. Corrigida a sobre-normalização de Flow/Form nesses mesmos três modelos: `FlowNode`/`FlowConnection`/`FormField`/`UserTaskConfig`/`IntegrationTaskConfig` eram documentados como tabelas/campos próprios (com PK/FK) quando a persistência real é um item de array dentro de coluna JSONB (`flow.nodes`/`flow.connections`/`flow.annotations`, `form.fields`) — ressalva adicionada em cada seção, mesmo padrão que `FlowAnnotation` já tinha; de quebra, corrigidas 3 fabricações sem base no código (`DisplayOrder`, `ConnectorEnabled`, `InputMapping` como campo próprio) e a nomenclatura `Form Component`→`Form Field` no modelo conceitual (regressão do que já tinha sido corrigido em 2026-08-09). Corrigidas colunas fantasma de `Form` (`code`/`status` não existem; `name` documentado como VARCHAR(200)/80, real é 150) no modelo físico e no `openapi.yaml` — como consequência, removidos filtro `status` de `GET /forms` e `409 Conflict` de `POST`/`PUT /forms`, sem base real sem a coluna `code`. No `openapi.yaml`, `IntegrationTaskConfig`/`RestConnectorConfig`/`KafkaConnectorConfig` reescritos pra bater com a forma real de `ConnectorConfig` (confirmada independentemente em `admin/back`, `admin/front` e `ms-espec-registry`, incluindo o trail de log de falha de execução `SimulationController.describeRequest`): removida a união discriminada REST/Kafka em favor de um único campo `config` livre, removido `inputMapping`, `outputMapping` movido pra dentro de `config`, `FlowNode.integrationTaskConfig` renomeado pra `connectorConfig` (nome real da propriedade), `query`→`params`, `PUBLISH`→`PRODUCE`, `clusterId` adicionado (faltava), `queue`/`correlationKey` removidos (fabricados). Sem mudança de contagem de REQs/features — só documentação de arquitetura/dados. |
