...
| [x] | REQ-05.10.009 | Uma mensagem Kafka recebida (Receive Task ou Início por Mensagem) deve ficar disponível para consulta, como já acontece pro lado produtor. | done | back: `KafkaConnectorWorker.consume()` (`ms-runtime-camunda`) passou a gravar `__kafkaTopic__<nodeId>`/`__kafkaPayload__<nodeId>` antes de correlacionar/iniciar a instância, simétrico ao `publishAndComplete()` do lado produtor — lido por `InstanceHistoryController.kafkaInput()` (`ms-espec-registry`), que já esperava essas variáveis | bug real corrigido: o consumo nunca gravava essas variáveis, então o payload recebido sempre aparecia vazio no painel/histórico |
