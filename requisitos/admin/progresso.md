...
| [x] | REQ-05.02.005 | O sistema deve apresentar a tela de uma User Task com toda referência `{{nome}}` já resolvida — na mensagem (sem tela) ou em qualquer prop de texto de um campo de tela real, inclusive pré-preenchendo `defaultValue` (editável). | done | `ms-espec-registry`: `StepResolver.resolveMessage`/`messageSdui` (caso sem tela) e `resolveSduiNode`/`resolveSduiValue` (caso com tela real — percorre toda a árvore SDUI); front: `SduiFormRenderer.collectInitialValues` usa `defaultValue` já resolvido como valor inicial do campo | reescrito nesta sessão (2026-08-24) — antes só funcionava no caso sem tela; bug real corrigido (tela real chegava com token literal) |
