...
| [x] | REQ-02.01.006 | O sistema deve impedir a desativação ou a exclusão de uma jornada enquanto sua publicação estiver ativa; o usuário deve despublicá-la antes. | done | back: `DeactivateJourney` e `DeleteJourney` reaproveitam a mesma guarda `ActivePublicationPort.existsForJourney` (409 `ActivePublicationExistsException`) | testado via curl: 409 ao tentar excluir jornada `PUBLISHED` |
