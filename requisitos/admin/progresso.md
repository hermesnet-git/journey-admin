...
| FT-04 | Formulários (SDUI) | 37 | 37 | 100% |
...
| [x] | REQ-04.01.004 | O sistema deve permitir usar um formulário do catálogo como modelo de partida ao desenhar a tela de uma User Task (cópia dos campos, sem vínculo persistido). | done | back: `FlowNode.embeddedScreen` (substitui `formId`); front: editor de tela embutido (`FormPreviewDock.tsx`/`FormScreenCanvas.tsx`) com ação "Importar formulário" que copia `Form.fields` pro nó | reescrito nesta sessão (2026-08-24) — Runtime Engine só suporta ~5-6 tipos de campo nativos, formId por User Task foi abandonado |
...
| [x] | REQ-04.01.006 | No editor de tela embutido, o sistema deve permitir importar campos de um formulário existente do catálogo e, separadamente, salvar a tela atual como um novo formulário reutilizável. | done | front: `FormPreviewDock.tsx` — ações "Importar formulário" (copia campos) e "Salvar como formulário reutilizável" (`promoteEmbeddedScreen`); back: `POST /journeys/{id}/flow/nodes/{nodeId}/promote-form` (`PromoteEmbeddedScreen`) | reescrito nesta sessão (2026-08-24) — substitui o antigo fluxo de "criar formulário sem sair do editor" |
| [x] | REQ-04.01.007 | No catálogo de formulários, cada campo deve possuir um `name` técnico único dentro do formulário e imutável após criado. Na tela embutida de uma User Task, o `name` é editável a qualquer momento, com unicidade verificada na jornada inteira (REQ-03.09.011). | done | back: `FormField.name`; `Form.create` valida nomes únicos no catálogo (`DuplicateFieldNameException`, 422); `FlowValidator` valida unicidade na jornada inteira pra campos de `embeddedScreen`; front: campo "Nome técnico" travado em `FieldCard` (catálogo), editável com validação em `FormFieldConfigPanel.tsx` (tela embutida) | catálogo testado via curl (201/422); tela embutida — validação de formato + unicidade cross-node adicionada nesta sessão (2026-08-24) |
...
| # | REQ | Descrição | Status | Evidência | Notas |
|---|---|---|---|---|---|
| [x] | REQ-04.03.001 | O sistema deve permitir usar um formulário do catálogo como ponto de partida (cópia) para telas de User Tasks em múltiplas jornadas. | done | back: `Form` é uma entidade independente, sem vínculo de jornada; cópia de campos pro `embeddedScreen` do nó, sem referência persistida | reescrito nesta sessão (2026-08-24) |
| [x] | REQ-04.03.002 | O sistema deve permitir usar um formulário do catálogo como ponto de partida (cópia) para telas de múltiplas User Tasks. | done | back: idem — múltiplos `FlowNode` podem copiar campos do mesmo `Form`, sem vínculo entre si | reescrito nesta sessão (2026-08-24) |
