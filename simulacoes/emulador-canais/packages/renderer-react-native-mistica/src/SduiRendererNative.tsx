...
function coercedInputValue(node: SduiNode, value: string): unknown {
  if ((node.attributes.inputMode === 'number' || node.attributes.inputMode === 'decimal') && value !== '') return Number(value);
  return value;
}
...
function NativeSelectField({
  ...
  const options = Array.isArray(node.attributes.options) ? node.attributes.options : [];
  ...
  const label = runtime.resolveText(node.attributes.label);
  const placeholder = runtime.resolveText(node.attributes.placeholder) || 'Selecione';

  if (node.attributes.searchable !== true) {
    return (
      <View style={styles.field}>
        <Text style={[styles.label, { color: tokens.colors.textPrimary }]}>{label}{node.attributes.required === true ? ' *' : ''}</Text>
        <View style={[styles.input, styles.picker, { borderColor: error ? tokens.colors.negative : tokens.colors.border }]}>
          <Picker enabled={active} accessibilityLabel={label} selectedValue={selectedValue} onValueChange={(value) => void onChange(node, value)} style={{ color: tokens.colors.textPrimary }}>
            <Picker.Item label={placeholder} value="" enabled={node.attributes.required !== true} />
      ...
      <Modal animationType="slide" presentationStyle="pageSheet" visible={visible} onRequestClose={() => setVisible(false)}>
        <SafeAreaView style={[styles.modal, { backgroundColor: tokens.colors.surface }]}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled">
            {node.attributes.required !== true && !query
...
function NativeDatePickerField({
  ...
  const mode = node.attributes.mode ?? 'date';
  const [visible, setVisible] = useState(false);
  ...
  return (
    <View style={styles.field}>
      <Text style={[styles.label, { color: tokens.colors.textPrimary }]}>{runtime.resolveText(node.attributes.label)}{node.attributes.required === true ? ' *' : ''}</Text>
      <Pressable
        accessibilityRole="button"
        disabled={!active}
        accessibilityLabel={runtime.resolveText(node.attributes.label)}
      ...
      >
        <Text style={{ color: runtime.getNodeValue(node) ? tokens.colors.textPrimary : tokens.colors.textSecondary }}>{displayDate(runtime.getNodeValue(node), mode, node.attributes.format) || 'Selecionar'}</Text>
      </Pressable>
      {error ? <Text style={[styles.helper, { color: tokens.colors.negative }]}>{error}</Text> : null}
      {visible ? (
        <View style={Platform.OS === 'ios' ? styles.iosPicker : undefined}>
          <DateTimePicker
            value={selected}
            mode={pickerMode}
            minimumDate={dateLimit(node.attributes.minDate)}
            maximumDate={dateLimit(node.attributes.maxDate)}
...
export function SduiRendererNative({
  ...
  const renderChildren = (node: SduiNode, active: boolean): ReactNode[] => node.children.map((child) => (
    <Fragment key={child.id}>{renderNode(child, active)}</Fragment>
  ));
  ...
  const renderNode = (node: SduiNode, ancestorsActive = true): ReactNode => {
    ...
    const children = renderChildren(node, active);
    ...
    switch (node.type) {
      ...
      case 'ui.container':
        return <View style={{ alignSelf: 'center', backgroundColor: color(tokens, props.backgroundToken), borderRadius: spacing(tokens, props.borderRadiusToken), padding: spacing(tokens, props.paddingToken), width: '100%' }}>{children}</View>;

      case 'ui.stack': {
        const direction = props.direction === 'horizontal' ? 'row' : 'column';
        return <View style={{ alignItems: props.alignment === 'start' ? 'flex-start' : props.alignment === 'end' ? 'flex-end' : props.alignment === 'center' ? 'center' : 'stretch', flexDirection: direction, gap: spacing(tokens, props.spacingToken, 16) }}>{children}</View>;
      }
      ...
      case 'ui.text':
        return <Text numberOfLines={typeof props.maxLines === 'number' ? props.maxLines : undefined} style={[textPreset(node), { color: color(tokens, props.colorToken) ?? tokens.colors.textPrimary, textAlign: textAlign(props.align) }]}>{resolved(runtime.getNodeAttribute(node, 'text'))}</Text>;

      case 'ui.image': {
        const source = resolved(runtime.getNodeAttribute(node, 'source'));
        ...
        return <Image accessibilityLabel={resolved(runtime.getNodeAttribute(node, 'alt'))} source={{ uri: source }} resizeMode={resizeMode} style={[styles.image, typeof props.aspectRatio === 'number' ? { aspectRatio: props.aspectRatio } : null]} />;
      }
        ...
        return (
          <View style={styles.field}>
            ...
            <TextInput
              ...
              accessibilityState={{ disabled: props.readOnly === true || !active }}
              ...
              editable={props.readOnly !== true && active}
      ...
      case 'ui.checkbox': {
        ...
        return (
          <View style={styles.field}>
            <Pressable disabled={!active} accessibilityRole="checkbox" accessibilityState={{ checked: indeterminate ? 'mixed' : checked, disabled: !active }} accessibilityLabel={resolved(props.label)} onPress={() => void change(node, !checked)} style={styles.checkboxRow}>
      ...
      case 'ui.datePicker':
        return <NativeDatePickerField node={node} runtime={runtime} error={fieldError(node)} tokens={tokens} onChange={change} active={active} />;

      case 'ui.button': {
        const disabled = props.disabled === true || !active || submitting || props.loading === true;
      ...
      case 'ui.alert': {
        ...
        return (
          <View accessibilityRole={severity === 'negative' ? 'alert' : 'summary'} style={[styles.alert, { backgroundColor: palette.background, borderColor: palette.foreground }]}>
            <View style={{ flex: 1, gap: 4 }}>{runtime.getNodeAttribute(node, 'title') ? <Text style={{ color: palette.foreground, fontSize: 16, fontWeight: '700' }}>{resolved(runtime.getNodeAttribute(node, 'title'))}</Text> : null}<Text style={{ color: tokens.colors.textPrimary }}>{resolved(runtime.getNodeAttribute(node, 'message'))}</Text></View>
            {props.dismissible === true ? <Pressable disabled={!active} accessibilityRole="button" accessibilityState={{ disabled: !active }} accessibilityLabel="Fechar aviso" onPress={() => node.events?.onDismiss ? void dispatch(node, 'onDismiss') : runtime.dismiss(node.id)}><Text style={{ color: palette.foreground, fontSize: 22 }}>×</Text></Pressable> : null}
          </View>
        );
      }
